...
/**
 * Narrow bridge for ZPA's built-in modules to query an effective broad type category.
 */
object InternalSemanticTypeBridge {
    @ZpaInternalApi
    @JvmSynthetic
    fun effectivePlSqlType(node: SemanticAstNode): PlSqlType =
        when (val category = EffectiveSemanticTypeQuery.typeCategory(node)) {
