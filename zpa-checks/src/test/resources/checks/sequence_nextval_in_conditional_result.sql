...
begin
  ...
  select decode(1, 1, 0, sequence_one.nextval) into value from dual;
  select decode(1, 1, to_char(sequence_one.nextval), 0) into value from dual;
  ...
  select decode(1, 1, case sequence_one.nextval when 1 then 'a' else 'b' end, 'x') into value from dual;
